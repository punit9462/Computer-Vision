...
if __name__ == '__main__':
    ...
    cv2.imshow('frame', segmask)
    cv2.waitKey()
    example_output_gray = cv2.imread(
        '/Users/punit/SBU/Acads/Vision/Assignments_Submission/HW4-Segmentation/example_output.png',
        cv2.IMREAD_GRAYSCALE)
    x = RMSD(segmask, example_output_gray)  # 6.677 TODO: Tune the SLIC params


    # superpixels_out = superpixels.copy()
    # for i in range(superpixels.shape[0]):
    #     for j in range(superpixels.shape[1]):
    #         if graph_cut[superpixels[i][j]]:
    #             superpixels_out[i][j] = 255
    #         else:
    #             superpixels_out[i][j] = 0
    # cv2.imshow('frame', superpixels_out.astype(dtype=np.uint8))
    # cv2.waitKey()

    # example_output_gray = cv2.imread('/Users/punit/SBU/Acads/Vision/Assignments_Submission/HW4-Segmentation/example_output.png', cv2.IMREAD_GRAYSCALE)
    # x = RMSD(superpixels_out.astype(dtype=np.uint8), example_output_gray) # 7.120202
