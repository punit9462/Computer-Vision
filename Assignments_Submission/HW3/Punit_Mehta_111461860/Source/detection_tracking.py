...
def camshift_tracker(v, file_name):
    ...
    centre_x = c + (w / 2)
    centre_y = r + (h / 2)
    output.write("%d,%d,%d\n" % (frameCounter, centre_x,
                                 centre_y))  # Write as 0,pt_x,pt_y
    ...
    while (1):
        ...
        output.write("%d,%d,%d\n" %
                     (frameCounter, int(ret[0][0]),
                      int(ret[0][1])))  # Write as frame_index,pt_x,pt_y
        frameCounter = frameCounter + 1
...
def particle_tracker(v, file_name):
    ...
    f0 = particleevaluator(hist_bp, init_pos) * np.ones(
    ...
    while 1:
        ...
        im_w = frame.shape[1]
        im_h = frame.shape[0]
        particles = particles.clip(np.zeros(2), np.array(
            (im_w, im_h)) - 1).astype(int)
...
def is_face_found(track_window):
    if (track_window[0] == 0 and track_window[1] == 0 and
            track_window[2] == 0 and track_window[3] == 0):
        return False
    else:
        return True
...
def kalman_tracker(v, file_name):
    ...
    kalman = cv2.KalmanFilter(4, 2,
                              0)  # 4 state/hidden, 2 measurement, 0 control
    kalman.transitionMatrix = np.array([
        [1., 0., .1, 0.],  # a rudimentary constant speed model:
        [0., 1., 0., .1],  # x_t+1 = x_t + v_t
        [0., 0., 1., 0.],
        [0., 0., 0., 1.]
    ])
    kalman.measurementMatrix = 1. * np.eye(
        2, 4)  # you can tweak these to make the tracker
    kalman.processNoiseCov = 1e-6 * np.eye(
        4, 4)  # respond faster to change and be less smooth
    ...
    while (1):
        ...
        # write the result to the output file
        output.write("%d,%d,%d\n" %
                     (frameCounter, prediction[0],
                      prediction[1]))  # Write as frame_index,pt_x,pt_y
        frameCounter = frameCounter + 1
