...
def histogram_equalization(img_in):
    ...
    one_chan_image_b, one_chan_image_g, one_chan_image_r = cv2.split(
        img_in)  # split in channels
    ...
    img_out = cv2.merge((equal_one_chan_image_b, equal_one_chan_image_g,
                         equal_one_chan_image_r))
    return True, img_out
...
def Question2():
    ...
    input_image2 = cv2.imread(
        sys.argv[3],
        cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)  # Used for deconvolution
